...
public class AIPlayer {
    ...
    public enum Role { DEALER, SMALL_BLIND, BIG_BLIND, NONE }
     ...
     */
    public BettingRound.Action decide(int callAmount, int pot, ArrayList<Card> community, Role role) {
        ...
        if (callAmount == 0) {
            ...
            return RNG.nextDouble() < 0.15 ? BettingRound.Action.BET : BettingRound.Action.CHECK;
        }
        ...
        if (strength > raiseThresh && chips > callAmount * 2) return BettingRound.Action.RAISE;
        ...
        if (strength > 0.20 && RNG.nextDouble() < 0.20)      return BettingRound.Action.CALL;  // bluff ocasional
    ...
    private double evaluateHandStrength(ArrayList<Card> community) {
        ...
        return rank.ordinal() / (double)(PokerGame.HandRank.values().length - 1);
    }

    public int decideAmount(int minBet, int currentPot, ArrayList<Card> community, Role role) {
        double strength  = evaluateHandStrength(community);
        boolean isBlind  = (role == Role.SMALL_BLIND || role == Role.BIG_BLIND);
    ...
    public void resetRoundBet() { currentBet = 0; }
    ...
    public void setFolded(boolean f)    { this.folded = f; }
    public void setAllIn(boolean a)     { this.allIn  = a; }
    public void setRole(Role r)         { this.role   = r; }
    public void addCard(Card c)         { hand.add(c); }
    public void clearHand()             { hand.clear(); folded = false; allIn = false; currentBet = 0; }
}